...
test('Nuclear recommended rate is saved on activation independently of loaded fuel and coolant',()=>{const x=setup({open:true,data:{state:'off',temperature:300},coolant:0});let activate;context.MultiblockGenerator.handlePlayerInteract=(e,c,h)=>{activate=h.onActivate;};handler.onPlayerInteract({});const args={entity:x.entity,components:{air:2,gas_cell:1,fuel_assemblies:4,rod_control:2,heat_conductor:2},energyCap:1000,settings:{multiblock:{transfer_rate_ratio:20}},structure:{bounds:{min:{x:0,y:0,z:0},max:{x:4,y:4,z:4}}}};activate(args);const rate=JSON.parse(x.values.get('nuclearStats')).recommendedRate;assert(rate>0);x.tick();const label=x.items.get(25).nameTag;x.entity.coolant.type='custom';x.entity.coolant.value=1000;const state=JSON.parse(x.values.get('nuclearData'));state.fuelType='uranium';x.values.set('nuclearData',JSON.stringify(state));context.runtimes.delete(x.entity.id);x.tick();assert.equal(x.items.get(25).nameTag,label);assert.equal(context.api.getReactorData(x.entity).recommendedRate,rate);args.components.heat_conductor=4;activate(args);assert(JSON.parse(x.values.get('nuclearStats')).recommendedRate>rate);});
