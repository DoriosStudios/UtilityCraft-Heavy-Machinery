...
test('Recommended rate is persisted on activation and stays fixed through tank changes and reload',()=>{const x=setup({open:true,data:{state:'off',temperature:300},coolant:0});handler.onPlayerInteract({});const args={entity:x.entity,components:{air:2,gas_cell:1,fluid_cell:2,thermo_core:1,heat_conductor:4},energyCap:1000,settings:{multiblock:{transfer_rate_ratio:20}},structure:{bounds:{min:{x:0,y:0,z:0},max:{x:4,y:4,z:4}}}};activation(args);const rate=JSON.parse(x.values.get('reactorStats')).recommendedRate;assert(rate>0);x.tick();const label=x.items.get(25).nameTag;x.entity.coolant.type='heavy_water';x.entity.coolant.value=1000;x.entity.exhaust.value=x.entity.exhaust.cap;context.runtimes.delete(x.entity.id);x.tick();assert.equal(x.items.get(25).nameTag,label);assert.equal(context.api.getReactorInfo(x.entity).recommendedRate,rate);args.components.heat_conductor=8;activation(args);assert(JSON.parse(x.values.get('reactorStats')).recommendedRate>rate);});

test('Rebuilding after meltdown restores cold stopped defaults across runtime reload',()=>{const x=setup({open:true,data:{temperature:1199,rate:100,startedAtMs:123},coolant:0});x.tick();assert.equal(x.read().meltdownPending,true);assert.equal(x.read().state,'off');assert.equal(x.read().rate,1);pending.splice(0).forEach(fn=>fn());context.runtimes.delete(x.entity.id);handler.onPlayerInteract({});activation({entity:x.entity,components:{air:2,gas_cell:1,fluid_cell:2,thermo_core:1,heat_conductor:4},energyCap:1000,settings:{multiblock:{transfer_rate_ratio:20}},structure:{bounds:{min:{x:0,y:0,z:0},max:{x:4,y:4,z:4}}}});for(const [k,v]of Object.entries(config.initialReactorData))assert.equal(x.read()[k],v,k);assert.equal(new TemperatureStorage(x.entity).get(),300);assert(x.items.get(6).nameTag.includes('1 mB/t'));const lava=x.entity.lava.value;x.tick();assert.equal(x.entity.lava.value,lava);assert.equal(x.read().state,'off');assert.equal(x.read().temperature,300);});
