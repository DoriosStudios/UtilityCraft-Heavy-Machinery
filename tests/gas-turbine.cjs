...
test('runtime uses gas IO, conserves energy and avoids display writes with UI closed',()=>{
    const x=setup();for(let i=0;i<20;i++)x.tick();assert(x.owner.gas.value<200);assert.equal(x.owner.energy.value,(200-x.owner.gas.value)*512);assert.equal(x.owner.writes,0);assert.equal(x.counters.gasDisplay,0);assert.equal(x.counters.energyDisplay,0);assert.equal(x.counters.io,0);assert.equal(x.ioConfig.gases.inputs[0].id,'fuel');
});
