...
function createMultipleLiquids(entity, inv, tankCount) {
    if (tankCount === 1) {
...
export class Machinery {
    ...
    constructor(block, settings) {
        ...
        this.liquid = createMultipleLiquids(this.entity, this.inv, settings.liquidTypes ?? 1);
    ...
    displayEnergy() {
        ...
        const lore = [
            `§r§7  Stored Energy: ${formatEnergyToText(energy)}/${formatEnergyToText(energyCap)}`,

        ];

        let speed = this.trueRateSpeed ?? this.trueBurnSpeed
        if (speed > 0) {
            lore.push(`§r§7  Rate: ${formatEnergyToText(speed)}/t`)
        } else if (this.settings.energyCost) lore.push(`§r§7  Cost: ${formatEnergyToText(this.settings.energyCost)}`)
        ...
        for (let i = 0; i <= 2; i++) {
            ...
            const bar = new ItemStack(`twm:energy_bar_${segment}`);
            bar.setLore(lore);
...
let energyExpScore = {}
let energyCapScore = {}
let energyCapExpScore = {}

world.afterEvents.worldLoad.subscribe(() => {
    energyScore = getOrCreateObjective("energy", "Energy");
    energyExpScore = getOrCreateObjective("energyExp", "EnergyExp");
    energyCapScore = getOrCreateObjective("energyCap", "Energy Max Capacity");
    energyCapExpScore = getOrCreateObjective("energyCapExp", "Energy Max Capacity Exp");
})
...
export class EnergyManager {
    ...
    transferToNetwork(speed) {
        ...
        if (this.entity.typeId == 'utilitycraft:power_condenser') {
            this.entity.setDynamicProperty('transfering', transferred)
        }
...
try { world.scoreboard.addObjective('maxLiquids', 'Max Liquids') } catch { }
...
export class LiquidManager {
    ...
    /**
     * Sets the tank capacity (compressed into scoreboard + exponent)
     * and clamps the current stored amount to the new capacity.
     *
     * @param {number} capacity Absolute capacity in mB (>= 0).
     * @returns {{ cap: number, clamped: boolean }} Effective capacity and whether current was clamped.
     */
    setCap(capacity) {
        const newCap = Math.max(0, Math.floor(capacity || 0));
        const { score, exp } = this.compress(newCap);

        this.scores.liquidCap.setScore(this.scoreId, score);
        this.scores.liquidCapExp.setScore(this.scoreId, exp);

        const current = this.get();
        if (current > newCap) {
            // set() will also set type to 'empty' if the result is 0
            this.set(newCap);
            return { cap: newCap, clamped: true };
        }
        return { cap: newCap, clamped: false };
    }

    /**
     * Adds (or subtracts) liquid at the current exponent magnitude.
    * Converts `amount` to the current magnitude and adds directly;
    * if the compressed value over/underflows, uses set() to re-pack.
    *
    * @param {number} amount Absolute amount in mB (can be negative).
    */
    add(amount) {
        const exp = this.scores.liquidExp.getScore(this.scoreId) || 0;
        const multi = 10 ** exp;
        const delta = amount / multi;

        const cur = this.scores.liquid.getScore(this.scoreId) || 0;
        ...
        this.scores.liquid.addScore(this.scoreId, delta);

        if ((this.scores.liquid.getScore(this.scoreId) || 0) === 0) {
            this.setType('empty');
        }
    }
    ...
    display(startSlot) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            item.setLore([
                `§r§7  Stored ${displayName}: ${formatLiquid(amount)}/${formatLiquid(cap)}`,
