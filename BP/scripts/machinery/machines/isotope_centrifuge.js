...
registerIOInterface("utilitycraft:isotope_centrifuge", {
    ...
    gases: {
        ...
        modes: [
            ...
            { id: "output_3", outputIndices: [0] },
            { id: "disabled" },
        ],
    },
});

DoriosLib.registry.blockComponent("utilitycraft:isotope_centrifuge", {
    ...
    onTick({ block }, { params: settings }) {
        ...
        const [inputGas, gas1, gas2] = GasStorage.initializeMultiple(machine.entity, 3);
        machine.processIO();

        const recipe = isotopeCentrifugeRecipes[inputGas.getType()];
        if (!recipe) {
            machine.showWarning("No Recipe");
            updateUI(machine, inputGas, gas1, gas2);
            return;
        }

        const requiredGas = recipe.required_gas ?? 1000;
        ...
        if (processCount > 0) {
            inputGas.consume(requiredGas * processCount);
...
function updateUI(machine, inputGas, gas1, gas2) {
    inputGas.display(3);
    gas1.display(4);
    gas2.display(5);
    machine.displayProgress();
