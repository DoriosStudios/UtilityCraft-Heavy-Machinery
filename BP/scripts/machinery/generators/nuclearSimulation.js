...
function transferHeat(temperature, ticks, heatRate, input, coolantBudget, config) {
    let remaining = ticks, elapsed = 0, coolantHeat = 0
    const passive = input.conductance * NUCLEAR_THERMAL.passiveCoolingFraction
...
export function simulateNuclearReactor(input, config) {
    ...
    while (remaining > 1e-10 && !meltdown) {
        ...
        const thermal = transferHeat(temperature, duration,
